    ...
    <div className="group relative flex flex-col bg-white rounded-[24px] border border-border/40 p-4 transition-all duration-300 hover:shadow-[0_12px_40px_rgb(45,106,79,0.12)] hover:-translate-y-1">
      ...
      {badge && (
        <div className="absolute left-6 top-6 z-10">
          <Badge variant="secondary" className="bg-white/90 backdrop-blur-md hover:bg-white text-primary font-bold px-3 py-1 shadow-sm border-none">
            {badge}
          </Badge>
        </div>
      )}
      ...
      {/* Image Container */}
      <Link href={`/product/${id}`} className="relative aspect-square overflow-hidden rounded-[16px] bg-secondary/30 mb-5 block group-hover:bg-secondary/40 transition-colors">
        {image ? (
          <Image 
            src={image} 
            alt={name}
      ...
      <div className="flex flex-1 flex-col px-1">
        ...
        <Link href={`/product/${id}`} className="group-hover:text-primary transition-colors">
          <h3 className="font-bold text-lg leading-tight mb-1 tracking-tight">{name}</h3>
        </Link>
